...
export default function ArtistProfilePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const artistData = await mockDB.getArtist(resolvedParams.id);
                if (artistData) {
                    setArtist(artistData);
                    const projectData = await mockDB.getProjects(resolvedParams.id);
            ...
            <div className="relative h-[400px] w-full overflow-hidden">
                ...
                <div className="absolute inset-0">
                    <Image
                        ...
                        alt="Background"
                        fill
                        className="object-cover opacity-30 blur-3xl scale-110" // heavy blur for atmosphere
                    />
