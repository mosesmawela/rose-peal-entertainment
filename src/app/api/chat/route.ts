...
// Cache prebuilt strings for system instruction to optimize performance
const PREBUILT_SITE_PAGES = SITE_PAGES.map(page => `- ${page.title} (${page.path}): ${page.description}`).join('\n');
const PREBUILT_ARTISTS = ARTISTS.map(artist => `- ${artist.name}: ${artist.bio} | Genres: ${artist.genre.join(', ')} | Latest: ${artist.latestRelease || 'N/A'} | Booking: ${artist.bookingAvailable ? 'Available' : 'Contact for availability'}`).join('\n');
const PREBUILT_FAQS = FAQS.map(faq => `Q: ${faq.question}\nA: ${faq.answer}`).join('\n\n');
