...
// Pre-compute static knowledge base strings to improve performance
const PRECOMPUTED_SITE_NAVIGATION = SITE_PAGES.map(page => `- ${page.title} (${page.path}): ${page.description}`).join('\n');
const PRECOMPUTED_ARTISTS = ARTISTS.map(artist => `- ${artist.name}: ${artist.bio} | Genres: ${artist.genre.join(', ')} | Latest: ${artist.latestRelease || 'N/A'} | Booking: ${artist.bookingAvailable ? 'Available' : 'Contact for availability'}`).join('\n');
const PRECOMPUTED_FAQS = FAQS.map(faq => `Q: ${faq.question}\nA: ${faq.answer}`).join('\n\n');
